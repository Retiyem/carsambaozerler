...
function populatePlayerPerformances(team, performances) {
    ...
    performances.forEach(perf => {
        ...
        playerCard.className = 'player-performance-card';
        
        const playerName = getPlayerNameById(perf.playerId);
        const playerRating = enhancedPlayers?.find(p => p.id === perf.playerId)?.rating || 75;
        
        playerCard.innerHTML = `
            <div class="player-performance-header">
                <div class="player-info">
                    <span class="player-name">${playerName}</span>
                    <span class="player-rating">Rating: ${playerRating}</span>
                </div>
...
function addMatchClickEvents() {
    ...
    matchTable.addEventListener('click', (e) => {
        ...
        if (!row || !row.dataset.matchId) return;
